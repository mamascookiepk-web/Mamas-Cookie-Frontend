...
export default function AdminTestimonialDetailModal({
  testimonial,
  onClose,
  onChangeStatus,
  submitting,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/50 p-4">
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-200 p-5">
          <div>
            <h2 className="text-lg font-extrabold text-ink-900">{testimonial.name}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <dl className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
            <div>
              <dt className="font-bold text-ink-900">Company</dt>
              <dd className="text-ink-600">{testimonial.companyName}</dd>
            </div>
            <div>
              <dt className="font-bold text-ink-900">Position</dt>
              <dd className="text-ink-600">{testimonial.position}</dd>
            </div>
            <div>
          ...
          <div className="mt-5">
            <p className="text-sm font-bold text-ink-900">Review</p>
          ...
          <div className="flex flex-wrap gap-2">
            {STATUSES.map((status) => (
              <button
                key={status}
                type="button"
                disabled={submitting || status === testimonial.status}
