...
export default function AdminGiftingDetailModal({ request, onClose, onChangeStatus, submitting, error }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/50 p-4">
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-200 p-5">
          <div>
            <h2 className="text-lg font-extrabold text-ink-900">{request.name}</h2>
          ...
          <dl className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
            ...
            {request.company && (
              <div>
                ...
                <dd className="text-ink-600">{request.company}</dd>
              </div>
            )}
            {request.eventDate && (
              <div>
            ...
            {request.budget != null && (
              <div>
                <dt className="font-bold text-ink-900">Budget</dt>
                <dd className="text-ink-600">{formatCurrency(request.budget)}</dd>
              </div>
            )}
          </dl>

          <div className="mt-5">
            <p className="text-sm font-bold text-ink-900">Requirements</p>
